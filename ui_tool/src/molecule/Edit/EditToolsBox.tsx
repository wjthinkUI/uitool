...
import { moveUpBlock, moveDownBlock, deleteBlock } from "@store/slice/sliceEditPage"
...
export const EditToolsBox = ({ block_id }: ToolsPropsType) => {
    ...
    return (
        <div className='absolute top-[30px] right-[100px] w-[246px] h-[54px] rounded-full bg-grayscale-600 hidden group-hover:flex z-20 justify-evenly pr-4 pl-4'>
            <button onClick={() => console.log('write')}><IconPencil className='fill-white hover:fill-primary-900' /></button>
            <button onClick={ReDesignSelect}><IconReset className='fill-white hover:fill-primary-900' /></button>
            <button onClick={() => MoveUp(block_id)}><IconUp className='fill-white hover:fill-primary-900' /></button>
            <button onClick={() => MoveDown(block_id)}><IconDown className='fill-white hover:fill-primary-900' /></button>
            <button onClick={() => Trash(block_id)}><IconTrashCan className='fill-white hover:fill-primary-900' /></button>
            {modal && <ModalBlockDesign blockIndex={block_id} closeModal={() => setModal(false)} />}
        </div>
    )
}