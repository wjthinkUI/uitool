...
export const ManageMenuListTree = () => {
  const storeData = useSelector((state: any) => state.pagesinfo.data.navigations);
  ...
  const mappingData = useMemo(() => dataMappingFlat(storeData), [storeData]);
      ...
      <div className="w-[1220px] h-auto p-8 rounded-[26px] bg-grayscale-50 border border-grayscale-200 border-dashed">
        <div className="flex flex-col items-end gap-2">
          {mappingData.map((value) => (
            (value.isParent) ? (<ListParentsMenu key={value.idx} title={value.name} path={value.children[0].path} category={value.name} date={value.date} isParent={value.isParent} />) : (<ListChildrenMenu key={value.idx} title={value.name} path={value.path} category={value.name} date={value.date} isParent={value.isParent} />))
          )}
        </div>
      </div>
    </>
